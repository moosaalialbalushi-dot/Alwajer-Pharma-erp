...
import {
  ...
  BrainCircuit, History, ChevronRight, Sparkles, Inbox,
} from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { id: 'intake',     label: 'Smart Intake',      icon: Inbox },
  { id: 'ai',         label: 'AI Command',        icon: BrainCircuit },
  { id: 'skills',     label: 'Skill Creator',     icon: Sparkles },
...
export const Sidebar: React.FC<Props> = ({ activeTab, onTabChange }) => (
  <aside className="hidden lg:flex flex-col w-56 shrink-0 bg-white border-r border-gray-200 h-screen sticky top-0 overflow-y-auto custom-scrollbar shadow-sm">
    {/* Logo */}
    <div className="flex flex-col items-center py-6 px-4 border-b border-gray-200">
      <div className="w-10 h-10 rounded-xl bg-[#D4AF37]/15 flex items-center justify-center mb-2">
        <span className="text-[#D4AF37] font-black text-sm">AWP</span>
      </div>
      <h1 className="text-slate-900 font-black text-xs tracking-widest uppercase">Al Wajer</h1>
      <p className="text-slate-500 text-[9px] tracking-widest uppercase mt-0.5">Pharma ERP v2</p>
    </div>
    {/* Nav */}
        ...
        return (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all text-sm font-medium
              ${active
                ? 'bg-[#D4AF37] text-slate-950 font-bold shadow-md shadow-[#D4AF37]/30'
    ...
    <div className="p-3 border-t border-gray-200">
      <p className="text-[10px] text-slate-500 text-center tracking-wider">© 2026 Al Wajer Pharma</p>
    </div>
  </aside>
);
