    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {SAMPLE_STAGES.map(stage => {
          ...
          return (
            <div key={stage} className="bg-white shadow-sm border border-[#D4AF37]/20 p-3 rounded-xl text-center">
      ...
      <div className="space-y-3">
        ...
        {samples.map(sample => {
          ...
          return (
            <div key={sample.id} className="bg-white shadow-sm border border-[#D4AF37]/30 rounded-xl p-5 gold-glow">
                ...
                <div className="flex justify-between mt-2">
                  {SAMPLE_STAGES.map((stage, i) => (
                    <div key={stage} className="text-center" style={{ width: '20%' }}>
                      <div className={`w-2 h-2 rounded-full mx-auto mb-1 ${i <= idx ? 'bg-[#D4AF37]' : 'bg-gray-200'}`}/>
                      <p className={`text-[8px] font-bold ${i === idx ? 'text-[#D4AF37]' : i < idx ? 'text-slate-500' : 'text-slate-400'}`}>
                        {stage}
                      </p>
                    </div>
                  ))}
