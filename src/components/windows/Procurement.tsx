              ...
              : shortages.map(item => (
                <div key={item.id} className="p-4 bg-red-500/10 rounded-lg border border-red-500/20 flex justify-between items-center mb-3">
                  ...
                  <button
                    ...
                    className="text-xs bg-red-500/20 hover:bg-red-500 text-red-400 hover:text-slate-900 px-3 py-1.5 rounded-lg border border-red-500/30 font-bold transition-all"
                  >
                    Fill Needs
                  </button>
                </div>
          ...
          <div className="bg-white shadow-sm border border-[#D4AF37]/30 rounded-xl p-5 gold-glow">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {MARKET_RATES.map(m => (
                  <tr key={m.name} className="hover:bg-gray-100 transition-colors">
                    <td className="py-2.5 text-slate-700 text-xs font-medium">{m.name}</td>
            ...
            {vendors.map(v => (
              <div key={v.id} className="p-4 bg-gray-50/30 rounded-lg border border-gray-200 hover:border-[#D4AF37]/20 transition-all group">
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-200">
                  <p className="text-[10px] text-slate-500">Materials: <span className="text-slate-700">API, Pellets</span></p>
                  <p className="text-[10px] text-right text-slate-500">Lead Time: <span className="text-slate-700">15-20d</span></p>
                </div>
              </div>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-gray-200">
              <h2 className="text-base font-bold text-slate-900">Generate Purchase Order</h2>
              <button onClick={() => setIsPOOpen(false)} className="text-slate-500 hover:text-slate-900 p-1.5 rounded-lg">✕</button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1.5">Material</label>
                <input value={poItem?.name ?? ''} onChange={() => {}} readOnly
                  className="w-full bg-gray-50 border border-gray-200 text-slate-900 rounded-lg px-3 py-2.5 text-sm"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1.5">Quantity ({poItem?.unit ?? 'kg'})</label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1.5">Preferred Vendor</label>
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-gray-200">
              <button onClick={() => setIsPOOpen(false)} className="px-4 py-2 text-sm font-bold text-slate-600 bg-gray-100 rounded-lg">Cancel</button>
              <button onClick={() => {
