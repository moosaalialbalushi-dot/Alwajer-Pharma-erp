...
export const AuditHistory: React.FC<Props> = ({ auditLogs, onClear }) => {
  ...
  const getColor = (action: string) => {
    ...
    return ACTION_COLOR[prefix] ?? 'text-slate-600';
  };
    ...
    <div className="space-y-5 animate-fadeIn">
        ...
        <div className="flex gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="bg-gray-50 border border-gray-200 text-slate-900 rounded-lg pl-8 pr-3 py-2 text-sm focus:border-[#D4AF37]/50 focus:outline-none w-44"
            />
          </div>
          <button onClick={() => exportToCSV(auditLogs as unknown as Record<string, unknown>[], 'audit_log')} className="erp-btn-ghost">
            <Download size={13}/>
          </button>
          <button onClick={onClear} className="p-2 text-slate-500 hover:text-red-400 bg-gray-100 hover:bg-red-500/10 rounded-lg border border-gray-200 transition-all" title="Clear all">
            <Trash2 size={14}/>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="bg-white shadow-sm border border-[#D4AF37]/20 p-3 rounded-xl">
        ...
        {paginated.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-200 text-[10px] text-slate-500 uppercase">
              ...
              <tbody className="divide-y divide-gray-200">
                {paginated.map(log => (
                  <tr key={log.id} className="hover:bg-gray-100 transition-all">
                    ...
                    <td className="py-3 px-4 text-slate-600 text-xs hidden sm:table-cell">{log.user}</td>
                    <td className="py-3 px-4 text-slate-700 text-xs max-w-[300px] truncate">{log.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <div className="flex gap-1">
              {Array.from({ length: Math.min(pages, 7) }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-7 h-7 text-xs font-bold rounded-lg transition-all ${page === p ? 'bg-[#D4AF37] text-slate-950' : 'bg-gray-100 text-slate-600 hover:text-slate-900'}`}>
                  {p}
                </button>
              ))}
            </div>
