...
const App: React.FC = () => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, context: 'global' | 'procurement' | 'industrial' | 'brainstorm' | 'rd' | 'bd' = 'global') => {
    ...
    reader.onload = async () => {
      ...
      if (context === 'industrial') {
          // Store for editing context instead of adding to chat immediately
      ...
      try {
        ...
        if (context === 'rd' || context === 'bd') {
             alert(`${context.toUpperCase()} File Processed: ${analysis.substring(0, 100)}...`);
        } else {
  ...
  const handleModalSave = async () => {
    ...
    if (modalType === 'add') {
         ...
         if (currentSection === 'accounting') setExpenses([...expenses, newItem]);
         if (currentSection === 'hr') setEmployees([...employees, newItem]);
                  ...
                  <tr className={`hover:bg-white/5 transition-all duration-200 ${expandedBatchId === batch.id ? 'bg-white/5' : ''}`}>
                    ...
                    <td className="py-4 px-4 text-right">
                        <div className="flex justify-end gap-2">
                            <button onClick={() => openModal('view', 'production', batch)} className="p-1.5 rounded hover:bg-blue-500/20 text-blue-400"><Eye size={14}/></button>
                            <button onClick={() => openModal('edit', 'production', batch)} className="p-1.5 rounded hover:bg-yellow-500/20 text-[#F4C430]"><Edit2 size={14}/></button>
                        </div>
                    </td>
                  </tr>
  ...
  const renderInventory = () => {
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="bg-slate-900/50 border border-[#D4AF37]/30 rounded-xl p-6 gold-glow">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
             {displayItems.length === 0 ? <p className="text-slate-500 text-sm">No items found.</p> : displayItems.map(item => {
               const percentage = Math.min((item.stock / item.safetyStock) * 100, 100);
               const isCritical = (item.balanceToPurchase && item.balanceToPurchase > 0) || item.stock < item.safetyStock;
               return (
                 <div key={item.id} className="bg-slate-800/50 p-4 rounded-lg border border-white/5 group relative hover:border-[#D4AF37]/50 transition-all">
                   <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openModal('edit', 'inventory', item)} className="p-1 rounded bg-slate-900 text-[#F4C430] hover:bg-yellow-900"><Edit2 size={12}/></button>
                   </div>
                   <div className="flex justify-between items-start mb-2">
                     <span className="text-[10px] text-slate-500 uppercase font-bold">{item.category}</span>
                     <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${isCritical ? 'bg-red-500/20 text-red-500' : 'bg-green-500/20 text-green-500'}`}>
                       {isCritical ? 'Shortage' : 'Good'}
                     </span>
                   </div>
                   <h4 className="text-white font-bold text-sm mb-1 truncate">{item.name}</h4>
                   <div className="flex justify-between items-end mb-2">
                       <p className="text-xl font-bold text-white">{item.stock} <span className="text-xs text-slate-500">{item.unit}</span></p>
                   </div>
                   <div className="w-full bg-slate-700 h-1.5 rounded-full overflow-hidden">
                      <div className={`h-full ${isCritical ? 'bg-red-500' : 'bg-green-500'}`} style={{ width: `${percentage}%` }} />
                   </div>
                 </div>
               );
             })}
              ...
              {orders.map(order => (
                <tr key={order.id} className="hover:bg-white/5">
                  ...
                  <td className="p-4 text-right">
                    <button onClick={() => openModal('edit', 'sales', order)} className="p-1.5 rounded hover:bg-yellow-500/20 text-[#F4C430]"><Edit2 size={14}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6`}>
                {activeStats.map((stat, idx) => (
                    <div key={idx} className="bg-slate-900/50 border border-[#D4AF37]/30 p-6 rounded-xl gold-glow flex flex-col justify-between h-32">
                        <stat.icon className={`${stat.color} mb-2`} size={24} />
                        <div>
                            <h3 className="text-slate-400 text-xs mb-1 uppercase tracking-widest font-bold">{stat.title}</h3>
              ...
              {expenses.map(exp => (
                <tr key={exp.id} className="hover:bg-white/5">
                  ...
                  <td className="py-4 text-right">
                      <button onClick={() => openModal('edit', 'accounting', exp)} className="text-[#D4AF37]"><Edit2 size={14}/></button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {employees.map(emp => (
            <div key={emp.id} className="p-4 bg-slate-800/30 rounded-lg border border-white/5 flex justify-between items-center group hover:border-[#D4AF37]/40 transition-all">
              ...
              <div className="text-right flex flex-col items-end">
                ...
                <button onClick={() => openModal('edit', 'hr', emp)} className="text-[#D4AF37] opacity-0 group-hover:opacity-100 transition-opacity"><Edit2 size={12}/></button>
              </div>
            </div>
          ))}
        </div>
              ...
              <div className="bg-slate-900/50 border border-[#D4AF37]/30 rounded-xl p-6 gold-glow">
                  ...
                  <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                      {filteredRD.length === 0 ? <p className="text-slate-500 text-sm">No formulations found.</p> : filteredRD.map(project => (
                          <div key={project.id} 
                               onClick={() => setSelectedRD(project)}
                               className={`p-4 rounded cursor-pointer border transition-all ${selectedRD?.id === project.id ? 'bg-[#D4AF37]/10 border-[#D4AF37]' : 'bg-slate-800/30 border-white/5 hover:bg-slate-800/50'}`}>
                              <div className="flex justify-between items-start">
                                  <h4 className="font-bold text-white">{project.title}</h4>
                                  <span className="text-[10px] bg-slate-950 text-slate-300 px-2 py-0.5 rounded">{project.status}</span>
                              </div>
                              <p className="text-[10px] text-slate-500 mt-1">ID: {project.id} • Score: {project.optimizationScore}</p>
                          </div>
                      ))}
                  </div>
              </div>
  ...
  return (
    <div className="h-screen w-full flex bg-[#020617] text-slate-200 overflow-hidden font-inter">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-950 border-r border-[#D4AF37]/20 flex flex-col transition-transform duration-300 md:relative md:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-8 h-full flex flex-col">
          <div className="flex justify-between items-center mb-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-white tracking-tight leading-none">ALWAJAR</h1>
                <span className="text-[10px] font-bold text-[#D4AF37] tracking-[0.2em] uppercase">Pharma ERP</span>
              </div>
            </div>
          ...
          <div className="pt-4 mt-4 border-t border-white/10">
              ...
              >
                  <Settings size={20}/>
                  <span className="text-sm font-bold">Settings & API</span>
              </button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar flex flex-col bg-[#020617] h-full">
        <header className="sticky top-0 h-20 bg-slate-950/80 backdrop-blur-md border-b border-white/5 px-8 flex items-center justify-between z-10 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-white tracking-tight">{activeTab === 'history' ? 'AUDIT HISTORY' : activeTab.toUpperCase() + ' HUB'}</h1>
          </div>
          <button onClick={handleGlobalAction} className="luxury-gradient px-4 py-2 rounded-lg text-slate-950 text-sm font-bold flex items-center gap-2">
            <Upload size={18} /> GLOBAL SYNC
          </button>
