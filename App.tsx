...
<table style="border:2px solid #000;border-top:none">
  ...
  const renderSales = () => {
    ...
    const statusColor: Record<string,string> = {
      ...
      'Going to Dispatch': 'text-cyan-400 bg-cyan-400/10',
      'Shipped': 'text-indigo-400 bg-indigo-400/10',
      'Delivered': 'text-green-400 bg-green-400/10',
      'Existing Orders': 'text-purple-400 bg-purple-400/10',
    ...
    const pendingValue = orders.filter(o => ['Pending','Processing'].includes(o.status)).reduce((s, o) => s + (o.amountUSD || 0), 0);
      ...
      <div className="space-y-5 animate-fadeIn pb-8">
        <div className="flex flex-wrap gap-3 justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={() => exportToCSV(orders, 'sales_report')}
          ...
          <table className="w-full text-left min-w-[900px]">
            <thead>
              <tr className="border-b border-white/10 bg-slate-900">
                {['Invoice / Date','Customer / Country','Product / Qty','Rate $','Amount $','OMR','Status','Actions'].map(h => (
                  <th key={h} className="px-4 py-3 text-[10px] text-slate-400 uppercase font-bold whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
              ...
              {orders.length === 0 ? (
                ...
                <tr key={order.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-4 py-3">
                    <div className="font-bold text-white text-sm font-mono">{order.invoiceNo}</div>
                  ...
                  <td className="px-4 py-3 max-w-[200px]">
                    <div className="text-sm text-slate-300 truncate">{order.product}</div>
                    <div className="text-xs font-bold text-white font-mono">{(order.quantity||0).toLocaleString()} KG</div>
                  </td>
                  <td className="px-4 py-3 text-sm font-mono text-slate-300">${(order.rateUSD||0).toFixed(2)}</td>
                  <td className="px-4 py-3 text-sm font-bold font-mono text-green-400">${(order.amountUSD||0).toLocaleString()}</td>
                  <td className="px-4 py-3 text-xs font-mono text-slate-500">{(order.amountOMR||0).toLocaleString()}</td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase whitespace-nowrap ${statusColor[order.status] || 'text-slate-400 bg-slate-400/10'}`}>
                      {order.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
        ...
        <div className="p-5 h-full flex flex-col">
          ...
          <div className="pt-4 mt-4 border-t border-white/10">
            <button onClick={() => { setIsSettingsOpen(true); setIsMobileMenuOpen(false); }}
              ...
              <Settings size={18}/><span className="text-sm font-bold">Settings & API Keys</span>
            </button>
          </div>
        </div>
      </aside>
